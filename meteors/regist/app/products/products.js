...
function readProductsTemplate(data, keywords) {
    ...
    var read_products_html = `
        ...
        <!-- <div id='CSRF' class='btn btn-secondary pull-right m-b-20px cross-site-Request-Forgery'>
        Get All Data
        </div>  -->
    ...
    $.each(data.records, function (key, val) {
        ...
        read_products_html += `<tr>
            ...
            <td>
                <!-- read product button -->
                <button class='btn btn-primary m-r-10px read-one-product-button' data-id='` + val.id_event + `'>
                    <span class='glyphicon glyphicon-eye-open'></span> Read
                </button>
 
                <!-- edit button -->
                <button class='btn btn-info m-r-10px update-product-button' data-id='` + val.id_event + `'>
